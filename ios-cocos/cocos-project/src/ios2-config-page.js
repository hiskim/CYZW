...
(function (global) {
    ...
    parts.config = {
        _showConfig: function () {
            ...
            var rowGap = Math.max(36, Math.min(82, Math.floor((firstRowY - 70) / 6)));
            ...
            option(firstRowY - rowGap, 'WebKit 游戏实例', String(webGameInstances), function () {
            ...
            option(firstRowY - rowGap * 2, 'WebKit 启动方式', webStartupMode === 'parallel' ? '并行启动' : '串行启动', function () {
            ...
            option(firstRowY - rowGap * 3, '显示 FPS', showFPS ? '开' : '关', function () {
                if (storage) storage.setItem('ios2.showFPS', showFPS ? '0' : '1');
                self._setNativePerformance('showFPS', showFPS ? 0 : 1);
                self._showConfig();
            });
            option(firstRowY - rowGap * 4, '登录后恢复性能设置', autoRestore ? '开' : '关', function () {
                if (storage) storage.setItem('ios2.autoRestore', autoRestore ? '0' : '1');
                self._showConfig();
            });
            option(firstRowY - rowGap * 5, '目标帧率', frameRate + ' FPS', function () {
                var values = ['30', '45', '60'];
            ...
            option(firstRowY - rowGap * 6, 'HSDK 详细日志', hsdkVerboseDebug ? '开' : '关', function () {
