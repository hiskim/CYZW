...
- **多开矩阵**：9:16 严格比例自动适配（闭式解 + 单调剪枝），启动并发闸门
  ...
  侧栏可收起（把整窗让给画面）；**实例全部关闭后自动把侧栏放回来**——空舞台上
  藏着的只有唯一入口，留着没意义（触发条件是从「有实例」到「零实例」这一次跳变，
  本来就空着时不动，免得上一次主动收起被立刻弹回）。
- **能耗仲裁**：焦点实例满帧出声，非焦点降帧（15 FPS）静音（帧率走
  「暂停 → 等旧循环退出 → 重启」路径，规避 Cocos 2.4.9 `setFrameRate` 缺陷）。
  设置页改档即时生效（`broadcastFrameRateChange()` 重放仲裁），改档回执落日志
  （`[instance] frame rate apply: ok-restart:30`）。⚠️ 90/120 档受显示器刷新率封顶
  （引擎对非 30/60 档用「setTimeout 对齐 rAF」，rAF 最快一次 vsync）。
- **帧率角标**（设置页 · 目标帧率）：开关 + **实测读数**（`焦点账号 · FPS 58/60`，
  绿=达标/黄=有偏差/红=没跑满），读数来自页面回执的 `fps=实测/目标` 段，2s 主动取一次
  （只在角标开着 + 页面可见时；关掉即停）。角标本身画在实例画面左上角，计数锚点是
  `cc.director.mainLoop` 而**不是** rAF——30 档是「rAF 照跑、隔帧才出帧」，非 30/60 档
  还会把全局 rAF 换成 setTimeout 版本，数 rAF 只能读到刷新率。斜杠后是
  `cc.game.config.frameRate`（宿主改档写的就是它），所以它就是「档位生效没有」的现场证据。
...
- **游戏加强**（独立分节页「游戏增强」，五张卡）：
  ...
  - **战斗数据**：战斗里血条上方三行「攻 / 盾 / 血」、怒气条下方一行「怒」
    （`攻 #FFD45A` `盾 #66D9FF` `血 #FF7777` `怒 #D99BFF`，≥1 万显示「万」、≥1000 万「亿」）。
    参考官方 APK 的 `builtin-battle-stats-overlay-apk.js`：挂钩
    `SystemHeadBoard.prototype._updateLifeAndRage(entity)`，用「包装 `entity.getComponent`
    一个调用周期」的方式借游戏的手拿组件（不用猜组件类名），再用官方角色名文本框
    的样式在血条容器上克隆标签并把名字藏起来。**省略**了参考实现的三套重型机制：
