...
enum LobbyComposition {
    ...
    /// **每次改动宿主侧行为就 bump 一次。**
    static let buildTag = "2026-09-23.47"
    ...
    static func makeSession() -> LobbySessionModel {
        ...
        LobbyLog.info("[lobby] build %@ | scriptRuntime=on downloadDelegate=on openURL=on accountProfile=on serverProfile=on packetCapture=on | webInspector=%@",
                      buildTag,
        ...
        // ⚠️ capture / commandCatalog / saltField 必须是**同一个实例**同时交给实例工厂
        // 与会话门面：实例收到帧要 ingest 到它，UI 开窗口也要开在它上面——
        // 分裂 = 「窗口正常弹出但一条数据都收不到」的静默故障（capture 踩过）。
        let commandCatalog = GameCommandStore()
        let capture = PacketCaptureController(catalog: commandCatalog)
        let saltField = SaltFieldChartController()
...
struct GameLobbyApp: App {
    ...
    var body: some Scene {
        WindowGroup(id: "MainLobby") {
            LobbyRootView(session: session, buildTag: LobbyComposition.buildTag)
        ...
        .commands {
            CommandMenu("诊断") {
                ...
                Button("抓取全部实例诊断快照") {
                    Task { @MainActor in await session.captureRenderAudit(allInstances: true) }
                }
                .keyboardShortcut("d", modifiers: [.command, .shift])

                Divider()

                Text("单个：先点一下那个窗口，再按 ⇧⌘A")
                Text("全部：⇧⌘D —— 一次抓所有实例，快照同一时刻、可直接横向对比")
            }
        }
    }
}
